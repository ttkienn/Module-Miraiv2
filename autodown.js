...
module.exports.downLoad = function (url, type, api, event) {
    ...
    this.getLink(url).then(res => {
        if (type == 'mp4') url = res.result.video.hd || res.result.video.sd || res.result.video.nowatermark || res.result.video.watermark;else if (type == 'mp3') url = res.result.music.play_url;
        axios({
            method: "GET",
            url: url,
...
module.exports.getLink = function (url) {
    return new Promise((resolve, reject) => {
        axios({
            method: "GET",
            url: `https://nguyenmanh.name.vn/api/autolink?url=${ url }&apikey=X7qFMoKB`
